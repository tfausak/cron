  ...
  testGroup
    ...
    [ testCase "matches a catch-all" $
      ...
      testCase "matches a specific field" $
        scheduleMatches
          stars {hour = mkHourSpec' (Field (SpecificField' (mkSpecificField' 1)))}
          (day 5 25 1 2)
          @?= True,
      testCase "matches a range" $
        scheduleMatches
          stars {dayOfMonth = mkDayOfMonthSpec' (Field (RangeField' (mkRangeField' 3 5)))}
          (day 5 4 1 2)
          @?= True,
      testCase "matches a list" $
        scheduleMatches
          stars {month = mkMonthSpec' (ListField (SpecificField' (mkSpecificField' 1) :| [SpecificField' (mkSpecificField' 2), SpecificField' (mkSpecificField' 3)]))}
          (day 2 3 1 2)
          @?= True,
      testCase "matches a step field" $
        scheduleMatches
          stars {dayOfMonth = mkDayOfMonthSpec' (StepField' (mkStepField' (RangeField' (mkRangeField' 10 16)) 2))}
          (day 5 12 1 2)
          @?= True,
      ...
      testCase "matches multiple fields at once" $
        scheduleMatches
          stars
            { minute = mkMinuteSpec' (StepField' (mkStepField' Star 2)),
      ...
      testCase "matches a monday as 1" $
        scheduleMatches
          stars {dayOfWeek = mkDayOfWeekSpec' (Field (SpecificField' (mkSpecificField' 1)))}
      ...
      testCase "only needs weekday or monthday to match" $
        ...
        scheduleMatches
          stars
            { dayOfWeek = mkDayOfWeekSpec' (Field (SpecificField' (mkSpecificField' 1))),
      ...
      testProperty "exact time matches" $ property $ do
        ...
        let (_, m, d, h, mn) = timeComponents t
            sched =
              CronSchedule
                (mkMinuteSpec' (Field (SpecificField' (mkSpecificField' mn))))
  ...
  testGroup
    ...
    [ testProperty "is always in the future (at least 1 minute advanced)" $ property $ do
      ...
      testCase "does not match impossible dates (restricted dow/dom bug)" $ do
        ...
        let cs =
              stars
                { month = mkMonthSpec' (Field (SpecificField' (mkSpecificField' 9))),
