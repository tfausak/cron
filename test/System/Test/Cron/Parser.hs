...
describeCronSchedule =
    ...
    [ testCase "parses all stars" $
      ...
      testCase "parses specific values" $
        assertSuccessfulParse
          "1 2 3 * *"
          stars
            { minute = mkMinuteSpec' (Field (SpecificField' (mkSpecificField' 1))),
  ...
  where
    assertSuccessfulParse = assertParse cronSchedule
...
describeCronScheduleLoose =
  ...
  where
    assertSuccessfulParse = assertParse cronScheduleLoose
...
describeCrontab =
  ...
  where
    assertSuccessfulParse = assertParse crontab
...
describeCrontabEntry =
  ...
  where
    assertSuccessfulParse = assertParse crontabEntry

describeSerializeParseCronSchedule :: TestTree
describeSerializeParseCronSchedule =
  testGroup
    "serialize/parse CronSchedule"
