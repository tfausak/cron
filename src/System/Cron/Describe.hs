...
describeCronField d (StepField' sf) =
  Concrete $
...
describeTime tf (minuteSpec -> m) (hourSpec -> h) =
  Other
    (return $ describeCronField minuteDescriptor m)
...
description t c =
  Desc
    (describeTime t (minute c) (hour c))
    (return ddom)
    (return dm)
    (return ddow)
  where
    ddom = describeCronField domDescriptor $ dayOfMonthSpec (dayOfMonth c)
